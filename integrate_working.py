...
# Extract everything after </style> for overview (it has no body/html wrapper)
...
# Remove <div class="claw-overview"> wrapper
overview_body = re.sub(r'^<div[^>]*class="claw-overview"[^>]*>', '', overview_body)
overview_body = re.sub(r'</div>\s*$', '', overview_body).strip()

# For evolution - extract between <body> and </body>
evolution_match = re.search(r'<body[^>]*>(.*?)</body>', evolution, re.DOTALL)
evolution_body = evolution_match.group(1).strip() if evolution_match else ""
# Remove outer wrapper div
evolution_body = re.sub(r'^<div[^>]*>', '', evolution_body)
evolution_body = re.sub(r'</div>\s*$', '', evolution_body).strip()
evolution_css = ''.join(re.findall(r'<style>(.*?)</style>', evolution, re.DOTALL))

# For trading - already extracted successfully
trading_match = re.search(r'<main[^>]*>(.*?)</main>', trading, re.DOTALL)
trading_body = trading_match.group(1).strip() if trading_match else ""
# Clean up trading body - remove the tab-content wrapper
        ...
        <main class="main">
            ...
            <div id="edge" class="tab-content">
                <div style="padding: 40px; text-align: center; color: var(--text-secondary);">
                    ...
                    <p style="margin-top: 20px;">Markets undervalue weather volatility by 12%</p>
                </div>
            </div>
            <div id="system" class="tab-content">
                <div style="padding: 40px; text-align: center; color: var(--text-secondary);">
                    <h2>System Status</h2>
                    <p style="margin: 20px 0;">Core Files: SOUL.md ✓ MEMORY.md ✓ AGENTS.md ✓ USER.md ✓</p>
                    <p style="font-size: 36px; font-family: var(--font-mono); color: var(--amber-500); margin: 20px 0;">C+ → B-</p>
                    <p>Gap 1: Autonomous decisions (45% → 80%)</p>
                    <p>Gap 2: Edge discovery (0 found)</p>
                </div>
            </div>
        </main>
    </div>
...
print(f"\nFinal dashboard: {len(final):,} characters")
print("All tabs integrated!")
